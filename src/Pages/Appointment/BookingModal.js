...
const BookingModal = ({ treatment, date, setTreatment, refetch }) => {
  ...
  const handleBooking = e => {
    ...
    fetch("https://a-dentist-server.onrender.com/booking", {
      method: "POST",
      headers: {
      ...
      .then(data => {
        if (data.success) {
          ...
          toast.error(
            `You already have an appointment Today at ${data?.booking.time}`
          );
        }
        refetch();
        setTreatment(null);
        ...
        <div className="modal-box ">
          ...
          {date ? (
            <form
              ...
              className="grid grid-cols-1 gap-y-6 justify-items-center mt-12 "
            >
              <input
                type="text"
                readOnly
